...
SELECT a.Company,
      ...
      a.Sale_Unit_Price * a.vat_rate Base_Sale_Unit_Price,
       a.Discount,       
      nvl(CASE when (b.Part_no IS NULL OR INVOICE_API.Is_Correction_Invoice(a.Company,a.Invoice_Id) = 'TRUE') OR (a.invoice_type IN ('CUSTORDCRE','CUSTCOLCRE') AND a.RMA_NO IS NULL)THEN 0 
      ...
      CASE WHEN a.sale_um = 'lb' then (a.Invoiced_Qty / e.conversion_factor) ELSE (a.Invoiced_Qty * e.conversion_factor) END as Base_Sales_Qty,
       /*  TO be tested       
       CASE when (b.Part_no IS NULL OR INVOICE_API.Is_Correction_Invoice(a.Company,a.Invoice_Id) = 'TRUE') THEN 0 
       ELSE CASE WHEN a.invoice_type IN ('CUSTORDCRE','CUSTCOLCRE') THEN a.Invoiced_Qty * -1 * e.conversion_factor 
       ELSE a.Invoiced_Qty * e.conversion_factor END 
       END Base_Sales_Qty,*/
