SELECT  
      ...
      nvl(CASE when (b.Part_no IS NULL OR INVOICE_API.Is_Correction_Invoice(a.Company,a.Invoice_Id) = 'TRUE') THEN 0 
       ELSE CASE WHEN a.invoice_type IN ('CUSTORDCRE','CUSTCOLCRE') AND (a.Sale_Unit_Price * a.Invoiced_Qty) > 0 THEN aaa.Cost * aaa.Conv_Factor * a.Invoiced_Qty * -1 
                 WHEN a.invoice_type IN ('CUSTORDCRE','CUSTCOLCRE') AND (a.Sale_Unit_Price * a.Invoiced_Qty) <= 0 THEN 0
                 ELSE aaa.Cost * aaa.Conv_Factor * a.Invoiced_Qty END
      end,0) Cost, 
      CASE 
      ...
      nvl(CASE  when b.Part_no IS NULL OR INVOICE_API.Is_Correction_Invoice(a.Company,a.Invoice_Id) = 'TRUE' THEN a.net_dom_amount 
                ELSE CASE WHEN a.invoice_type IN ('CUSTORDCRE','CUSTCOLCRE') AND (a.Sale_Unit_Price * a.Invoiced_Qty) > 0 THEN (a.net_dom_amount + (aaa.Cost * aaa.Conv_Factor * a.Invoiced_Qty)) 
                          WHEN a.invoice_type IN ('CUSTORDCRE','CUSTCOLCRE') AND (a.Sale_Unit_Price * a.Invoiced_Qty) <= 0 THEN a.net_dom_amount
                          ELSE (a.net_dom_amount - (aaa.Cost * aaa.Conv_Factor * a.Invoiced_Qty)) END
      ...
      ELSE (nvl(CASE when b.Part_no IS NULL OR INVOICE_API.Is_Correction_Invoice(a.Company,a.Invoice_Id) = 'TRUE' THEN a.net_dom_amount 
       ...
       CASE WHEN a.net_dom_amount = 0 THEN 0
      ELSE (nvl(CASE when b.Part_no IS NULL OR INVOICE_API.Is_Correction_Invoice(a.Company,a.Invoice_Id) = 'TRUE' THEN a.net_dom_amount 
                     ELSE CASE WHEN a.invoice_type IN ('CUSTORDCRE','CUSTCOLCRE') AND (aaa.Cost * aaa.Conv_Factor * a.Invoiced_Qty) > 0 THEN (a.net_dom_amount + (aaa.Cost * aaa.Conv_Factor * a.Invoiced_Qty)) 
                               WHEN a.invoice_type IN ('CUSTORDCRE','CUSTCOLCRE') AND (aaa.Cost * aaa.Conv_Factor * a.Invoiced_Qty) <= 0 THEN a.net_dom_amount
                               ELSE (a.net_dom_amount - (aaa.Cost * aaa.Conv_Factor * a.Invoiced_Qty)) END
...
left join (select customer_id, address_id, district_code, Region_code from CUST_ORD_CUSTOMER_ADDRESS_ENT where address_id = 'MAIN') h on a.identity = h.customer_id
   ...
   --AND a.Company = '20-MEX'
   AND trunc(ahead.Print_Date) = TRUNC(SYSDATE) - 1
   AND a.Invoice_Id = '196474'
